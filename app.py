    ...
    notifications.append(
        {
            ...
            "message": "В реальной системе банк мог бы считать нового клиента как держателя карты, который активировал предложение и оплатил у партнёра впервые за выбранный период.",
            "type": "info",
        }
    )
...
def summarize_metrics(df: pd.DataFrame) -> dict:
    if df.empty:
        return {"new_clients": 0, "revenue": 0.0, "transactions": 0, "avg_check": 0.0, "repeat_clients": 0}
    return {
...
if st.session_state.user_type == "client":
    ...
    if tab == "🗺 Мои траты":
        ...
        notifications = [
            ("💰", "Вы часто тратите на кафе — партнёры рядом могут помочь сэкономить до 500 ₽ в месяц.", "active"),
